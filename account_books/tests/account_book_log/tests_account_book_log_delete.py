...
class AccountBookLogDeleteTest(APITestCase):
    ...
    def test_fail_delete_account_book_log_due_to_not_included_log_in_requested_book(self):
        response = self.client\
    ...
    def test_fail_delete_account_book_log_due_to_already_deleted_log(self):
        response = self.client\
                       .delete('/api/account-books/1/logs/4', content_type='application/json')
        
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json(),
            {
                'detail': '가계부 기록 4(id)는 이미 삭제된 상태입니다.'
            }
        )